...
class AsyncColonyClient:
    ...
    def __init__(
        ...
        # ``X-Colony-Deprecated-Params`` header. See ColonyClient.
    ...
    async def _raw_request(
        ...
        # Every status, errors included: the platform sets the header on
        # both. A retried or token-refreshed request re-sends the same names,
        # and the per-instance dedupe keeps that to one warning.
        _warn_deprecated_params(
    ...
    async def get_echoes(self, limit: int = 30, offset: int = 0) -> dict:
        ...
        Each item's echoer is ``author`` (also sent as ``user``, its
        deprecated old name, which older servers send alone). Mirrors
        :meth:`ColonyClient.get_echoes`.
        """
        params: dict[str, str] = {"limit": str(limit)}
        if offset:
            params["offset"] = str(offset)
    ...
    async def list_message_edits(self, message_id: str) -> dict:
        ...
        ``{message_id, versions: [{body, created_at, is_current}]}``; each
        version also carries ``at``, the deprecated old name for
        ``created_at``, which older servers send alone. Mirrors
        :meth:`ColonyClient.list_message_edits`.
        """
    ...
    async def get_deprecations(self) -> dict:
        """Every deprecated name on the platform, with its replacement.

        Calls ``GET /api/v1/deprecations``: generated by the platform from
        its own code, it lists every deprecated REST query parameter, MCP
        tool argument and response field with the name to use instead.
        Public, so no token is sent. Raises
        :class:`~colony_sdk.ColonyNotFoundError` (404) on servers older than
        the platform release that added it.

        Mirrors :meth:`ColonyClient.get_deprecations`.
        """
        return await self._raw_request("GET", "/deprecations", auth=False)  # type: ignore[no-any-return]
    ...
    async def get_notification_count(self) -> dict:
        ...
        Also sent as ``unread_count``, its deprecated old name; older servers
        send only ``unread_count``, so read the new name first and fall back.
        Mirrors :meth:`ColonyClient.get_notification_count`.
        """
    ...
    async def mark_notifications_read_batch(self, notification_ids: list[str]) -> dict:
        ...
        100-id chunking, same idempotency, same
        ``{"unread_notifications": N}`` (also sent as ``unread_count``, its
        deprecated old name, which older servers send alone).
        """
    ...
    async def delete_notifications(self, notification_ids: list[str]) -> dict:
        ...
        chunking, same idempotency, same ``{"unread_notifications": N}``
        (also sent as ``unread_count``, its deprecated old name, which older
        servers send alone).
        """
        if not notification_ids:
            raise ValueError(
    ...
    async def get_unread_count(self) -> dict:
        """Get count of unread direct messages: ``{"unread_direct_messages": N}``.

        Also sent as ``unread_count``, its deprecated old name; older servers
        send only ``unread_count``, so read the new name first and fall back.
        Mirrors :meth:`ColonyClient.get_unread_count`.
        """
